...
class Matrix extends Var {
    ...
    public Var add(Var other) throws CalcExeption {
        if (other instanceof Scalar) {
            Logger.getLogger().log("Запуск Операции сложения Матрица + Скаляр");
            ...
            for (int i = 0; i < res.length; i++) {
                for (int j = 0; j < res[0].length; j++) {
                    ...
                    Logger.getLogger().log(CalcLogOut.ADD + res[i][j] + "+" + other);
                }
            }
            return new Matrix(res);
        }
        else if (other instanceof Matrix){
            Logger.getLogger().log("Запуск операции сложения Матрица + Матрица");
            if (value.length == ((Matrix) other).value.length && value[0].length == ((Matrix) other).value[0].length) {
                ...
                for (int i = 0; i < res.length; i++) {
                    for (int j = 0; j < res[0].length; j++) {
                        ...
                        Logger.getLogger().log(CalcLogOut.ADD + res[i][j] + "+" + ((Matrix) other).value[i][j]);
                    }
                }
                return new Matrix(res);
    ...
    public Var sub(Var other) throws CalcExeption {
        if (other instanceof Scalar) {
            Logger.getLogger().log("Запуск операции вычитания Матрица - Скаляр");
            double[][] res = new double[value.length][0];
            for (int i = 0; i < res.length; i++) {
                res[i] = Arrays.copyOf(value[i], value[i].length);
            }
            for (int i = 0; i < res.length; i++) {
                for (int j = 0; j < res[0].length; j++) {
                    res[i][j] = res[i][j] - ((Scalar) other).getValue();
                    Logger.getLogger().log(CalcLogOut.SUB + res[i][j] + "-" + other);
                }
            }
            return new Matrix(res);
        } else if (other instanceof Matrix) {
            Logger.getLogger().log("Запуск операции вычитания Матрица - Матрица");
            if (value.length == ((Matrix) other).value.length && value[0].length == ((Matrix) other).value[0].length) {
                double[][] res = new double[value.length][0];
                for (int i = 0; i < res.length; i++) {
                    res[i] = Arrays.copyOf(value[i], value[i].length);
                ...
                for (int i = 0; i < res.length; i++) {
                    for (int j = 0; j < res[0].length; j++) {
                        res[i][j] = res[i][j] - ((Matrix) other).value[i][j];
                        Logger.getLogger().log(CalcLogOut.SUB + res[i][j] + "-" + ((Matrix) other).value[i][j]);
                    }
                }
                return new Matrix(res);
            } else throw new CalcExeption("Размеры матрицы не совпадают");
    ...
    public Var mul(Var other) throws CalcExeption {
        if (other instanceof Scalar) {
            Logger.getLogger().log("Запуск операции умножения Матрица * Скаляр");
            double[][] res = new double[value.length][0];
            for (int i = 0; i < res.length; i++) {
                res[i] = Arrays.copyOf(value[i], value[i].length);
            }
            for (int i = 0; i < res.length; i++) {
                for (int j = 0; j < res[0].length; j++) {
                    res[i][j] = res[i][j] * ((Scalar) other).getValue();
                    Logger.getLogger().log(CalcLogOut.MUL + res[i][j] + "*" + other);
                }
            }
            return new Matrix(res);
        } else if (other instanceof Vector) {
            Logger.getLogger().log("Запуск операции умножения Матрица * Вектор");
            if (value[0].length == ((Vector) other).getVector().length || value.length == ((Vector) other).getVector().length) {
                ...
                for (int i = 0; i < res.length; i++) {
                    for (int j = 0; j < vector.length; j++) {
                        ...
                        Logger.getLogger().log(CalcLogOut.MUL + res[i][j] + "*" + vector[j]);
                    }
                }
                return new Vector(matrixSize);
            } else throw new CalcExeption("Размеры матрицы и вектора не совпадают");
        } else if (other instanceof Matrix) {
            Logger.getLogger().log("Запуск операции умножения Матрица * Матрицу");
            if (value.length == ((Matrix) other).value.length && value[0].length == ((Matrix) other).value[0].length) {
                double[][] res = new double[value.length][0];
                ...
                for (int i = 0; i < res.length; i++) {
                    for (int j = 0; j < ((Matrix) other).value[0].length; j++) {
                        for (int k = 0; k < ((Matrix) other).value.length; k++) {
                            ...
                            Logger.getLogger().log(CalcLogOut.MUL + res[i][k] + "*" + ((Matrix) other).value[k][j]);
                        }
                    }
                }
        ...
        if (other instanceof Scalar) {
            if (((Scalar) other).getValue() != 0) {
                ...
                for (int i = 0; i < res.length; i++) {
                    for (int j = 0; j < res[0].length; j++) {
                        res[i][j] = res[i][j] / ((Scalar) other).getValue();
                        Logger.getLogger().log(CalcLogOut.DIV + res[i][j] + "/" + other);
                    }
                }
                return new Matrix(res);
    ...
    public Matrix(double[][] value) {
        Logger.getLogger().log("Копирование значений вектора");
    ...
    Matrix(String strMatrix) {
        Logger.getLogger().log("Преобразование строки в матрицу");
        String one = "";
        String two = "";
    ...
    public String toString() {
        Logger.getLogger().log("Преобразование матрицы в строку");
        ...
        Logger.getLogger().log("Result" + sb2);
        return sb2.toString();
    }
}
