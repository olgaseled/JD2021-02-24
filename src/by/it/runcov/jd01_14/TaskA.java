...
public class TaskA {
        ...
        try {
            dataInputStream = new DataInputStream(
                    new BufferedInputStream(
                            new FileInputStream(dataTaskAPath)));
    ...
    private static void printToTxtFile(List<Integer> list, String txtParn) {
        try (PrintWriter printWriter = new PrintWriter(txtParn)) {
            double sumForFile = 0;
            for (Integer integer : list) {
                sumForFile += integer;
            ...
            throw new RuntimeException("my massage", e);
        }
    }
}
